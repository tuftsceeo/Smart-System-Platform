...
recipient_mac = b'd\xe83\x84\x12\x80' #[b'\xec\xda;\xbe\x86\x90', b'\xec\xda;\xbe\x89 '] #
...
while True:
    ...
    print(f"Own sensor: {point}")
#     sensor_data = {}
#     sensor_data["sensor"] = point[0]
#     sensor_data["potentiometer"] = point[1]
#     for recipient_macs in recipient_mac:
#         networking.aen.send_sensor(recipient_macs, sensor_data)
    
    recv_sensor_data = {}
    try:
        print(networking.aen.received_data)
        recv_sensor_data = networking.aen.received_data[recipient_mac]
        print(f"Last received sensor data: {recv_sensor_data}")
    except Exception as e:
        print(f"Error {e}")
    
    try:
        if time.ticks_ms() <= recv_sensor_data["time_recv"] + 1000:#make this false and the SM becomes a master
            point = (recv_sensor_data["sensor"], recv_sensor_data["potentiometer"])
            print(f"Foreign sensor: {point}")
        else:
            print("Recevied data too old, using own data")
    except Exception as e:
        print(f"Error2 {e}")
